...
import {
  ...
  Building2, MapPin, ArrowRightLeft, Package, Truck, ListChecks, Clock, MessageSquare, FileText, Scale, FileDown,
} from 'lucide-react'
...
export default function OrdemServicoPoliDetalhe() {
  ...
  const enviarWhatsApp = async () => {
    ...
    setErro('')
    try {
      const r = await api.post(`/ordens-servico/poli/${os.id}/enviar-whatsapp`, {})
  ...
  return (
    <div className="p-4 md:p-8 animate-fade-in pb-32 max-w-4xl mx-auto">
      ...
      <div className="fixed bottom-0 left-0 right-0 md:left-60 bg-white border-t p-3 flex gap-2 z-30" style={{ borderColor: '#E0DDD8' }}>
        ...
        <button
          onClick={() => {
            api.get(`/ordens-servico/poli/${os.id}/pdf`, { responseType: 'blob' })
              .then((r) => window.open(URL.createObjectURL(r.data), '_blank'))
