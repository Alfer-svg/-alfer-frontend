                  ...
                  {Array.isArray(c.tiposAplicaveis) && c.tiposAplicaveis.length > 0 ? (
                    c.tiposAplicaveis.map((t: string) => (
                      <span key={t} className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: '#E3EEFA', color: '#1A5276' }}>
                        {({ CONTAINER_SECO: 'Seco', CONTAINER_REEFER: 'Reefer', CACAMBA_ESTACIONARIA: 'Caçamba', CAMINHAO_MUNCK: 'Munck', CAMINHAO_POLIGUINDASTE: 'Poliguindaste', CAMINHAO_CAVALO_MECANICO: 'Cavalo Mecânico' } as Record<string, string>)[t] || t}
                      </span>
                    ))
                  ) : (
