...
const movLabel: Record<string, string> = {
  ENTREGA_VAZIA: 'Entrega vazia',
  COLETA_CHEIA: 'Coleta cheia',
  TROCA: 'Troca',
  TRANSPORTE_DESTINO: 'Transporte → destino',
}

const fmtDateTime = (d?: string) =>
  d ? new Date(d).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' }) : '—'

export default function OrdensServicoPoli() {
  ...
  const load = () => {
    ...
    api
      .get('/ordens-servico/poli', { params })
  ...
  return (
    <div className="p-8 animate-fade-in">
      <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-gray-900">Ordens de Serviço — Poliguindaste</h1>
          <p className="text-gray-500 text-sm mt-1">{itens.length} OS — entrega/coleta/troca de caçambas + MTR</p>
        </div>
        <button
          onClick={() => navigate('/ordens-servico/poli/nova')}
        ...
        <div className="space-y-3 stagger">
          {itens.map((o) => {
            ...
            return (
              <div
                key={o.id}
                onClick={() => navigate(`/ordens-servico/poli/${o.id}`)}
              ...
              >
                ...
                >
                  <img src="/icones/poliguindaste.png" alt="Poliguindaste" className="w-9 h-9 object-contain"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1 flex-wrap">
                    ...
                    {o.tipoMovimento && (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: '#E3EEFA', color: '#1A5276' }}>
                        {movLabel[o.tipoMovimento] || o.tipoMovimento}
                      </span>
                    )}
                    {o.contrato && (
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500 flex-wrap">
                    ...
                    {o.cacambaCodigo && <span>Caçamba {o.cacambaCodigo}</span>}
                    {o.caminhao && <span>{o.caminhao.codigo} — {o.caminhao.placa}</span>}
                    {o.operador && <span>Motorista: {o.operador.nome}</span>}
